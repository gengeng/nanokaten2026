...
const CONFIG = {
  ...
  // ゲージ設定
  gaugeDuration: 60000,    // 0→90%の時間（ms）※duration_minuteから自動計算で上書き
...
async function loadRules() {
  try {
    ...
    // duration_minute からgaugeDurationを自動計算
    if (state.remoteConfig.durationMinute && state.rules.length > 0) {
      const totalMs = state.remoteConfig.durationMinute * 60 * 1000;
      const calculated = Math.floor(totalMs / state.rules.length);
      CONFIG.gaugeDuration = calculated;
      state.gaugeDuration = calculated;
      console.log(`Auto gaugeDuration: ${calculated}ms (${state.remoteConfig.durationMinute}min / ${state.rules.length} rules)`);
    }
  } catch (error) {
...
function scrollLeftPanelToElement(el) {
  ...
  // stickyヘッダーの高さを取得（ヘッダーに隠れないようオフセット）
  const leftHalf = el.closest('.left-half');
  const header = leftHalf?.querySelector('.left-half-header');
...
function startRemoteConfigPolling() {
  ...
  setInterval(async () => {
    try {
      ...
      // start_time到達でリロード
      if (config.startTime && !state.startTimeReloaded) {
        // Google SheetsのDate値をパース（シリアル値 or ISO文字列）
        let targetTime;
        if (typeof config.startTime === 'number') {
          // Excelシリアル値: (日数 - 25569) * 86400000
          targetTime = new Date((config.startTime - 25569) * 86400000);
        } else {
          targetTime = new Date(config.startTime);
        }

        const now = new Date();
        if (now >= targetTime) {
          console.log(`Start time reached: ${targetTime.toISOString()}`);
          state.startTimeReloaded = true;
